...
programIcon = PhotoImage(file='icons/app.png')
...
apkaless_pic = ImageTk.PhotoImage(Image.open('icons/apkaless-soldier.jpg'))
...
FileMenu.add_command(label='Open', command=open_file, image=openfile, compound='left')

FileMenu.add_command(label='Save As', command=save_file, image=saveimg, compound='left')

FileMenu.add_separator()

FileMenu.add_command(label='Exit', image=extimg, compound='left')
...
textWidget.config(font=('Arial', 12), fg='black', undo=True, yscrollcommand=scroll.set, height=1080, width=1920)
...
result_Widget.pack(side='left', fill='x')
...
system_widget.bind('<Key>', lambda e: 'break')
